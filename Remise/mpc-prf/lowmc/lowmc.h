...
/** @} */ // end of group1
	...
	inline auto debug_substitute2_p2(block_t blind0, block_t blind1,  block_t blind2) const
	{
 
 		printf("debug_substitute2_p2 blind0 = %llu \n", blind0.mX[0]);
		printf("debug_substitute2_p2 blind1 = %llu \n", blind1.mX[0]);
		printf("debug_substitute2_p2 blind2 = %llu \n\n", blind2.mX[0]);
		...
		printf("debug_substitute2_p2 gamma0 = %llu \n", gamma0.mX[0]);
		printf("debug_substitute2_p2 gamma1 = %llu \n", gamma1.mX[0]);

		return std::make_pair(gamma0, gamma1);
	} // substitute2_p2


	inline auto debug_encrypt2_p0p1(const block_t & myshare, const block_t & othershare,  bool p)
	{
		auto c0 = myshare;
		auto c1 = othershare;
		...
		block_t blind0_, blind1_, blind2_;

 		blind0.mX[0] = 12211311; blind0.mX[1] = 0;
		blind1.mX[0] = 12343242; blind1.mX[1] = 0;
		blind2.mX[0] = 34424223; blind2.mX[1] = 0;
		...
		auto [gamma0, gamma1] = debug_substitute2_p2(blind0, blind1, blind2);
	
		#ifdef VERBOSE
		printf("debug gamma0 = %llu \n", gamma0.mX[0]);
		printf("debug gamma1 = %llu \n", gamma1.mX[0]);
		#endif

 		blind0_.mX[0] = 12211311; blind0_.mX[1] = 0;
		blind1_.mX[0] = 12343242; blind1_.mX[1] = 0;
		blind2_.mX[0] = 34424223; blind2_.mX[1] = 0;
		...
		auto [gamma0_, gamma1_] = debug_substitute2_p2(blind0_, blind1_, blind2_);
		...
		block_t blinded_c1 = othershare ^ blind1;
		block_t blinded_c0 = myshare ^ blind0;
		
		block_t blinded_c, blinded_c_recv;

		if(p)
		{
			blinded_c = myshare ^ blind1;
			blinded_c_recv = othershare ^ blind0;
		}

		if(!p)
		{
			blinded_c = myshare ^ blind0;
			blinded_c_recv = othershare ^ blind1;
		}
		...
		for (size_t i = 0; i < 1; ++i)
		{
			...
			if(!p) 
			{
			  c0 = substitute2_p0p1(c0, blind0, blinded_c_recv, gamma0);
			  //printf("debug 0 --> c0 = %llu\n", c0.mX[0]);
			}
			if(p)  
			{
			  c0 = substitute2_p0p1(c0, blind1, blinded_c_recv, gamma1);
			  //printf("debug 1 --> c0 = %llu\n", c0.mX[0]);
			}
	...
	inline auto encrypt2_p0p1(const block_t & share0,  
			streams::socket_stream<block_t> & sb,  bool p) const
	{
		...
		for (size_t i = 0; i < 1; ++i)
		{
			...
			if(p)
			{
				blind0.mX[0] = 16399217790659225674ULL;
				blind0.mX[1] = 6203238487763196238ULL; 
				gamma.mX[0] = 7168741446085949755ULL; 
				gamma.mX[1] = 5397407695172923705ULL; 
			 }

			 if(!p)
			 {
				blind0.mX[0] = 13416086074252806898ULL; 
				blind0.mX[1] = 13486746437209917037ULL; 
				gamma.mX[0] = 7168741443793726475ULL; 
				gamma.mX[1] = 5397407695172923705ULL; 
				
			 }
			
			 sb << (c0 ^ blind0);   //< blind own share; send to other party
			 sb >> blinded_c1;       //< get blinded share from other party
		//	s2 >> gamma;              //< get cancellation term from P2
	...
	inline void encrypt2_p2(streams::socket_stream<block_t>& s0, streams::socket_stream<block_t>& s1, randomness & rand0,
		randomness & rand1, randomness & rand2) const
	{
		for (size_t i = 0; i < rounds; ++i)
		{
			...
			s0 << gamma0;             //< send next cancellation term to P0
			s1 << gamma1;             //< send next cancellation term to P1

			// printf("gamma sent to P0 =  %llu <-> %llu\n", gamma0.mX[0], gamma0.mX[1]);
			// printf("gamma sent to P1 =  %llu <-> %llu\n\n", gamma1.mX[0], gamma1.mX[1]);
		}
	} // encrypt2_p2
...
/** @} */ // end of group2
 ...
 //    block_t maska = block_t(std::string(sbox_mask, 0, identity_len  - 1));
		...
		randomness & rand2) const
	{
		...
		printf("blind0.mX[0] = %llu\n", (unsigned long long) blind0.mX[0]);
		printf("blind1.mX[0] = %llu\n", (unsigned long long) blind1.mX[0]);
		printf("blind2.mX[0] = %llu\n", (unsigned long long) blind2.mX[0]);

		printf("blind0.mX[1] = %llu\n", (unsigned long long) blind0.mX[1]);
		printf("blind1.mX[1] = %llu\n", (unsigned long long) blind1.mX[1]);
		printf("blind2.mX[1] = %llu\n", (unsigned long long) blind2.mX[1]);

		printf("gamma0.mX[0] = %llu\n", (unsigned long long) gamma0.mX[0]);
		printf("gamma0.mX[1] = %llu\n", (unsigned long long) gamma0.mX[1]);

		printf("gamma1.mX[0] = %llu\n", (unsigned long long) gamma1.mX[0]);
		printf("gamma1.mX[1] = %llu\n", (unsigned long long) gamma1.mX[1]);
