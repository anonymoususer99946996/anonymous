...
class bitsliced_dpf_key
{
  public:
	...
	using finalizer_t = std::vector<std::array<node_t, 256>>;
	...
	// inline constexpr size_t full_bytes() { return interval_bytes(0, nitems-1); }
	 ...
	 inline constexpr size_t nodes_at_leaf_layer() const { return bitsliced_dpf_key::nodes_at_leaf_layer(nitems); }
  	 ...
  	 size_t nitems;
	 std::array<node_t, 256> root;
	
	// std::array<node_t, 256> cw_;	
	 
	 std::vector<std::array<node_t, 256>> cw;
...
template<typename row_t = __m128i, typename lowmc>
inline void PRG_bit_sliced(const lowmc & prgkey, const std::array<block<__m256i> , 128>& seed, void * outbuf, const uint32_t len)
{
	std::array<block<__m256i> , 128> * outbuf128 = reinterpret_cast<std::array<block<__m256i> , 128> *>(outbuf);
	std::array<block<__m256i> , 128> tmp = seed;
    outbuf128[0] = prgkey.encrypt(seed);

   	for(size_t j = 0; j < 128; ++j) outbuf128[0][j] ^= tmp[j];

   	const block<__m256i> ones = _mm256_set1_epi64x(-1);
	...
	for (size_t i = 1; i < len; ++i)
	{
        ...
        outbuf128[i] =  prgkey.encrypt(tmp);
  	    ...
  	    for(size_t j = 0; j < 128; ++j) outbuf128[i][j] ^= tmp[j];
	}
...
inline std::array<block<__m128i>, 128> bitsliced_clear_lsb(std::array<block<__m128i>, 128>& block, uint8_t bits = 0b11)
{
	if(bits == 0b11)
	{
...
inline std::array<block<__m256i>, 128> bitsliced_clear_lsb(std::array<block<__m256i>, 128>& block, uint8_t bits = 0b11)
{
	if(bits == 0b11)
	{
...
//  void print_first_col__(std::array<block<__m256i> , 128> inp, size_t col = 0)
// {
...
// }
 ...
 void print_first_col__(std::array<block<__m128i> , 128> inp, size_t col = 0)
{
...
template<typename prgkey_t, typename row_t = __m128i>
static inline void bitsliced_traverse2(
										 prgkey_t& prgkey, const std::array<block<row_t>, 128> & cw,  const block<row_t> cw_t[2], 
										 sliceblock_t& seed, sliceblock_t s[2], block<row_t> t[2], 
										 block<row_t> prev_t, int lsbmask = 0b11
										)
{
	 sliceblock_t seed_ = bitsliced_clear_lsb(seed, 0b11);	
	 
 

	 // print_first_col__(seed_);

	 PRG_bit_sliced(prgkey, seed_, s,2);

 
	 // printf("s[L]: ");
	 // print_first_col__(s[L]);
	 // printf("s[R]: ");
	 // print_first_col__(s[R]);
	 ...
	 {
	   s[L][j] =  xor_if(s[L][j].mX, cw[j].mX, (prev_t.mX ^ ones));  
	 }	
	 ...
	 for(size_t j = 0; j < 128; ++j)
	 {
	   s[R][j] =  xor_if(s[R][j].mX, cw[j].mX, (prev_t.mX ^ ones));  
	 }
...
std::array<block<__m128i>, 128> ___transpose(const std::array<__m128i, 128>& input)
{
  
  std::array<block<__m128i>, 128> inputT;
...
template<typename finalizer_t, typename prgkey_t>
static inline void stretch_leaf_bitsliced(const prgkey_t & prgkey, sliceblock_t seed, finalizer_t & s)
{
	sliceblock_t seed_ = bitsliced_clear_lsb(seed, 0b11);	
	PRG_bit_sliced(prgkey, seed_,  s,1);
} // dpf::stretch_leaf_bitsliced


//template<typename node_t, typename prgkey_t, typename row_t = __m128i>
template<typename leaf_t, typename node_t, typename prgkey_t, typename row_t = __m128i>
inline void bitsliced_finalize(const prgkey_t & prgkey, bitsliced_dpf_key<leaf_t, node_t, prgkey_t> & dpfkey, block<__m128i> * bitsliced_output, sliceblock_t * s, size_t nnodes, block<row_t> * t)
{
	auto output_ = reinterpret_cast<std::array<sliceblock_t, dpf_key<leaf_t, node_t, prgkey_t>::nodes_per_leaf> *>(bitsliced_output);
	
	for(size_t i = 0; i < nnodes; ++i)
	{
		stretch_leaf_bitsliced(prgkey, s[i], s);
		for(size_t j = 0; j < dpfkey.nodes_per_leaf; ++j)
		{ 
			for(size_t k = 0; k < 128; ++k)
			{
				output_[i][j][k] =  xor_if(output_[i][j][k], dpfkey.finalizer[k][j], t[i][k]);
			}
		}
	}
}
...
template<typename leaf_t, typename node_t, typename prgkey_t, typename row_t = __m128i>
inline void __evalinterval_bitsliced(const prgkey_t& prgkey, bitsliced_dpf_key<leaf_t, node_t, prgkey_t> & dpfkey, 
										   block<__m128i> * bitsliced_output, block<row_t>* flags, size_t from, size_t to)
{
	
	sliceblock_t root_array = ___transpose(dpfkey.root);
	// transpose(dpfkey.root);
 	sliceblock_t encrypted_root_array =  prgkey.encrypt(root_array);



 	printf("encrypted_root: new: \n");

 	// print_first_col__(encrypted_root_array);

 	// printf("\n\n\ntransposed root: \n");
 	// print_first_col__(root_array);
	...
	std::vector<sliceblock_t> cw_array;
	...
	sliceblock_t * s[2] = {
	    reinterpret_cast<sliceblock_t *>(bitsliced_output) + nodes_in_interval * (nodes_per_leaf - 1),
	...
	cw_array.reserve(depth + 1);	
 	printf("Printing the transposed CWs: \n"); 
 	for(size_t d = 0; d < depth; ++d)
 	{
 		cw_array[d] = ___transpose(dpfkey.cw[d]);

  		//print_first_col__(cw_array[d]);
 	}
 ...
 //	print_first_col__(s[curlayer][0]);
    ...
    bitsliced_finalize(prgkey, dpfkey, bitsliced_output, s[0], nodes_in_interval, t[0]); 
}
