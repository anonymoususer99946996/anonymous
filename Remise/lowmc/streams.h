...
inline socket_stream&
operator<<(const std::array<block<__m256i>, N>& val)
{
    boost::asio::write(
        socket,
        boost::asio::buffer(val.data(), N * sizeof(block<__m256i>))
    );
    return *this;
}
...
template <std::size_t N>
inline socket_stream&
operator>>(std::array<block<__m256i>, N>& val)
{
    boost::asio::read(
        socket,
        boost::asio::buffer(val.data(), N * sizeof(block<__m256i>))
